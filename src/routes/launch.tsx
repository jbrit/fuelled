...
import { arrayify, bn, createAssetId, hexlify, BytesLike, chunkAndPadBytes } from "fuels";
...
console.log({memeFactoryContractId})
...
function Launch() {
  ...
  const memeFactorycontract = wallet && new MemeFactory(memeFactoryContractId, wallet);
  ...
  const onLaunchPress = async () => {
    ...
    const { waitForResult, contractId } = await BondingCurveFactory.deploy(wallet, {storageSlots: BondingCurve.storageSlots});
    await waitForResult();
    const newBondingCurve = new BondingCurve(contractId, wallet);
    setNewCurve(newBondingCurve);
    try {
      await memeFactorycontract.functions.set_bytecode_root({bits: contractId}).call();
  ...
  const onFinishLaunchPress = async () => {
    ...
    try {
      ...
      console.log("registering child", {bits: newCurve.id.toB256()})
      await memeFactorycontract.functions.register_contract({bits: newCurve.id.toB256()}, name, symbol).addContracts([newCurve]).call();
      setAssetId(assetId);
    } catch (error) {
      console.log(error)
      // @ts-ignore
  ...
  return (
    <div className="flex flex-col gap-2 items-center">
        ...
        {!!newCurve && <>
        <div className="flex flex-col gap-2 items-center">
            Contract: {(newCurve.id.toHexString())}
          <div>
            Asset deployed at
          </div>
          <div>
